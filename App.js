...
import { View, Text, TextInput, StyleSheet, TouchableOpacity, FlatList, Pressable, Modal, Picker } from 'react-native';
    ...
    return(
		<View style={styles.container}>
			<Modal animationType="fade" transparent={true} visible={taskFormRemove}>
				<Pressable style={styles.moddalOverlay }>
					<View style={styles.modal}>
						<TouchableOpacity style={styles.modalButton} onPress={() =>{setTaskFormRemove(false)}}>
							<AntDesign name="close" size={24} color="black" />
						</TouchableOpacity>
						<Text style={styles.modalTitle}>Remove task</Text>
						<Text style={styles.modalText}>When processing the task will be removed from the list.</Text>

						<View style={styles.buttonActions}>
							<TouchableOpacity style={[styles.button, styles.buttonSubmit]} onPress={() =>{removeTask()}}>
								<Text style={styles.buttonSubmitText}>Remove</Text>
							</TouchableOpacity>
							<TouchableOpacity style={[styles.button, styles.buttonOutline]} onPress={() =>{setTaskFormRemove(false)}}>
			...
			<Modal animationType="fade" transparent={true} visible={taskFormAdd}>
				<Pressable style={styles.moddalOverlay }>
					<View style={styles.modal}>
						<TouchableOpacity style={styles.modalButton} onPress={() =>{setTaskFormAdd(false)}}>
							<AntDesign name="close" size={24} color="black" />
						</TouchableOpacity>
						...
						<View style={styles.inputContainer}>
							<TextInput
								...
								placeholder="Description" placeholderTextColor="#424242"
								underlineColorAndroid="transparent"
								onChangeText={(text) => setTaskTargetTitle(text)}
								/>
							<Feather style={styles.inputIcon} name="edit" size={24} color="black" />
						</View>
						<View style={styles.inputContainer}>
							...
							<Feather style={styles.inputIcon} name="alert-circle" size={24} color="black" />
						</View>
						<View style={styles.buttonActions}>
							<TouchableOpacity style={[styles.button, styles.buttonSubmit]} onPress={() =>{addTask()}}>
								<Text style={styles.buttonSubmitText}>Save</Text>
							</TouchableOpacity>
							<TouchableOpacity style={[styles.button, styles.buttonOutline]} onPress={() =>{setTaskFormAdd(false)}}>
								<Text style={styles.buttonOutlineText}>Cancel</Text>
							</TouchableOpacity>
						</View>
					</View>
			...
			<Modal animationType="fade" transparent={true} visible={taskFormEdit}>
				<Pressable style={styles.moddalOverlay }>
					<View style={styles.modal}>
						<TouchableOpacity style={styles.modalButton} onPress={() =>{setTaskFormEdit(false)}}>
							<AntDesign name="close" size={24} color="black" />
						</TouchableOpacity>
						...
						<View style={[styles.buttonActions, styles.buttonActionsBetween]}>
							<View style={{"flexDirection": "row"}}>
								<TouchableOpacity style={[styles.button, styles.buttonOutline]} onPress={() =>{setTaskFormEdit(false)}}>
									<Text style={styles.buttonOutlineText}>Cancel</Text>
								</TouchableOpacity>
								<TouchableOpacity style={[styles.button, styles.buttonSubmit]} onPress={() =>{editTask()}}>
									<Text style={styles.buttonSubmitText}>Save</Text>
								</TouchableOpacity>
							</View>
							<View>
								<TouchableOpacity style={[styles.button, styles.buttonSubmit]} onPress={() =>{setTaskFormEdit(false); setTaskFormRemove(true)}}>
									<Text style={styles.buttonSubmitText}>Remove</Text>
								</TouchableOpacity>
							</View>
						</View>
			...
			<View>
				...
				item.id}
				renderItem={(({item}) =>
				<Pressable onPress={() =>{setTaskTargetID(item.id); setTaskTargetTitle(item.title); setTaskTargetPriority(item.priority); setTaskFormEdit(true)}}>
					<View style={[styles.item, prioritys[item.priority].style]}>
						<View style={[styles.itemLeft, prioritys[item.priority].style]}>
							<Text style={[styles.priorityText, prioritys[item.priority].style]}>{prioritys[item.priority].text}</Text>
